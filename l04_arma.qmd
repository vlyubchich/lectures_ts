...
Based on the plots (@fig-simYtacf), *ACF decays faster than linearly* (some indication of stationarity), *PACF is non-zero up to lag 2* (cuts off after lag 2). Knowing the expected behavior of ACF and PACF for ARMA models (see @tbl-arma), ARMA(2,0) can be suggested in this case. 
...
However, the syntax of the package `fable` is quite different from its alternatives.

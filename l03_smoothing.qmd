...
Consider monthly data of Secchi disk depth measured at the station CB1.1 in Chesapeake Bay (@fig-Secchi), with the decomposition in @fig-SecchiDecompose.
...
#| fig-cap: "Shampoo sales over a three-year period smoothed with different exponential smoothing filters."
clrs <- c("0.3" = 4, "0.7" = 2)
...
Monthly totals of international airline passengers, 1949--1960 (see @fig-airpassangers). This is a classical example and benchmark in time series analysis.
...
The last multiplicative model is the best one, based on the sum of squared errors (SSE) in the training set (see @fig-airpassangersHW). For a more thorough comparison, consider using out-of-sample data in a cross-validation.
...
Model `tm2.1` uses a pre-calculated quadratic transformation, while model `tm2.2` applies the transformation on-the-fly within the R formula call, using the `I()` syntax (without the `I()` wrapper, the output of `tm2.2` would be the same as of `tm1`, not `tm2.1`). However, both models `tm2.1` and `tm2.2` can easily violate the assumption of independence of predictors in linear regression, especially when values of the time index $t$ are large. That is the case for us, because $t$ represents decimal years, from `r min(t)` to `r max(t)`, and $\widehat{\mathrm{cor}}(t, t^2) =$ `r cor(t, t2)`. Therefore, model `tm2.3` is preferred, because it is evaluated on orthogonal polynomials (basically, centering and normalization is applied to $t$ and $t^2$).
:::
...
In R, it is enough to have *one* variable representing the seasons. This variable should be saved as `factor`. Then we apply the OLS method to find the model coefficients (see results in @fig-airpassangersRegTrendSeas):
...
Based on @fig-AirPassengersDiff, differencing once with the non-seasonal and the seasonal lags is enough to remove the trend and strong seasonality. The final series (denoted as `D1D12`) is
$$
\begin{split}

...
1. Learn how to identify the orders $p$ and $q$ using ACF and PACF plots.
1. Practice model estimation using different methods and model selection using information criteria (AIC and BIC).
1. Practice diagnostics of residuals of these models.
1. Learn how to obtain forecasts from these models.
...
$$
S_t = \sum_{i = 1}^t X_i,\; \text{where}
$X_t \sim \mathrm{i.i.d.}(0, \sigma^2);\; \sigma = 2.
$${#eq-rw}
...
$$
S2_t = \sum_{i = 1}^t X_i,\; \text{where}\;
$X_t \sim \mathrm{i.i.d.}(a, \sigma^2);\; a = 0.2,\; \sigma = 2.
$${#eq-rw2}
...
ACF of AR(1) model takes two graphical forms depending on whether the coefficient $\phi_{1}$ is positive or negative. In the first case, the decay occurs through the positive axis only (@fig-ar1). In the second case, the decay alternates between the negative and positive axes (@fig-ar2). Although both forms are examples of exponential decay, they appear different visually and you must learn to recognize both.
...
<!-- Examples of ARMA models and their expansions: -->
<!-- \begin{itemize} -->
<!-- \item -->
<!-- \textbf{ARMA(1,2)}: short hand notation $\phi^{1} (B)Y_{t} = \theta^{2} (B) \epsilon_{t}$. Therefore -->
<!-- $$ -->
...
<!-- Isolating $Y_{t}$ on the left side of the equation gives the final form of the expansion -->
<!-- $$ -->
<!-- Y_{t} = \phi_{1} Y_{t -1} + \epsilon_{t} + \theta_{ 1} \epsilon_{t - 1} + \theta_{2} \epsilon_{t-2}. -->
<!-- $$ -->

<!-- \item -->
<!-- \textbf{ARMA(3,1)}: short hand notation $\phi^{3} (B)Y_{t} = \theta^{1} (B) \epsilon_{t}$. Therefore -->
<!-- $$ -->
...
<!-- In a mixed model ARMA($p, q$) process (that is, $p > 0$, $q > 0$), neither the ACF -->
...
<!-- \begin{itemize} -->
<!-- \item {\bf ACF:} $p$ initial spikes, then damps out as a mixed exponential decay of order $p$ (never 0) -->
<!-- \item {\bf PACF:} $p$ initial spikes, then cuts off; PACF($h$) = 0 for $h > p$ -->
<!-- \end{itemize} -->

<!-- \item -->
<!-- MA($q$) models: $Y_{t} = \theta^{q} (B) \epsilon_{t}$ -->
<!-- \begin{itemize} -->
<!-- \item {\bf ACF:} $q$ initial spikes, then cuts off; ACF($h$) = 0 for $h > q$ -->
<!-- \item {\bf PACF:} $q$ initial spikes, then damps out as a mixed exponential decay of -->
<!-- order $q$ (never 0) -->
<!-- \end{itemize} -->
...
<!-- \begin{itemize} -->
<!-- \item {\bf ACF:} has max ($p, q$) initial spikes, then damps out as a mixed exponential -->
<!-- decay driven by the AR($p$) component (never 0). -->
<!-- \item {\bf PACF:} has max($p, q$) initial spikes, then damps out as a mixed exponential -->
...
<!-- \begin{table} -->
<!-- \caption{Summary of ARMA($p, q$) models} -->
<!-- {#tab:summaryARMA} -->
<!-- \centering -->
<!-- \begin{tabular}{c c c} -->
<!-- \toprule -->
<!-- \textbf{Model}& \textbf{ACF} & \textbf{PACF} \\ \midrule -->
...
<!-- <<echo=F>>= -->
<!-- set.seed(1) -->
<!-- Yt <- arima.sim(list(order = c(2, 0, 1), ar = c(0.5, 0.3), ma = c(0.2)), n = T) -->
<!-- @ -->

<!-- Suppose you are given some data (a univariate time series) $Y_t$. -->
...
<!-- pacf(Yt, las = 1) -->
<!-- @ -->
<!-- \caption{Estimated ACF and PACF of $Y_t$.} -->
<!-- {#fig-simYtacf} -->
<!-- \end{figure} -->

<!-- \paragraph{Note} By default, R plots ACF starting from lag 0, and plots PACF starting from lag 1. Of course, ACF(0) = PACF(0) = 1. -->

<!-- Based on the plots (@fig-simYtacf}), \textbf{ACF decays faster than linear} (some indication of stationarity),\textbf{ PACF is non-zero up to lag~2} (cuts off after lag~2). Knowing the expected behavior of ACF and PACF for ARMA models (see Table~\ref{tab:summaryARMA}), ARMA(2,0) can be suggested in this case. Additionally, we can check (not too many) other combinations within the maximal lag you identified (2). Thus, we can check ARMA(1,0), (2,0), (0,1), (1,1), (2,1), (0,2), (1,2), and ARMA(2,2): -->

<!-- ```{r} -->
<!-- Results <- expand.grid(p = c(0,1,2), q = c(0,1,2))[-1,] #combinations of p and q, without p=q=0 -->
<!-- for(i in 1:nrow(Results)) { -->
<!-- mod <- arima(Yt, order = c(Results$p[i], 0, Results$q[i])) -->
...
<!-- The model with the lowest AIC uses the following $p$ and $q$: -->

<!-- ```{r} -->
<!-- m <- which.min(Results$aic) -->
<!-- p <- Results$p[m] -->
<!-- q <- Results$q[m] -->
<!-- c(p, q) -->
<!-- @ -->
...
<!-- Get the estimates of this model once again: -->

<!-- ```{r} -->
<!-- mod2 <- arima(Yt, order = c(p, 0, q)) -->
<!-- mod2 -->
<!-- ls(mod2) -->
<!-- @ -->
...
<!-- we can get the 'fitted' values, $\hat{Y}_t$ (technically, those are one-step-ahead predictions): -->
...
<!-- Plot the observed and fitted values (@fig-simYtfit}). -->
...
<!-- \begin{figure}[h] -->
<!-- ```{r} -->
<!-- plot.ts(Yt, las = 1, col = "grey50", lwd = 2, type = "o") -->
<!-- lines(Yhat, col = "blue", lwd = 2) -->
<!-- @ -->
<!-- \caption{Observed time series $Y_t$ (gray) and one-step-ahead predictions from ARMA(\Sexpr{p},\Sexpr{q}) model.} -->
<!-- {#fig-simYtfit} -->
<!-- \end{figure} -->


<!-- Model diagnostics are same as for usual regression: check residuals for homoskedasticity, uncorrelatedness, and normality (Figures~@fig-simYtdiag1}--@fig-simYtdiag3}). -->
...
<!-- tsdiag(mod2) -->
<!-- @ -->
<!-- \caption{Check residuals for homoskedasticity (top) and uncorrelatedness (middle and bottom).} -->
<!-- {#fig-simYtdiag1} -->
<!-- \end{figure} -->
...
<!-- pacf(e, las = 1) -->
<!-- @ -->
<!-- \caption{Check residuals for uncorrelatedness, using ACF and PACF plots.} -->
<!-- {#fig-simYtdiag2} -->
<!-- \end{figure} -->
...
<!-- qqline(e) -->
<!-- @ -->
<!-- \caption{Check residuals for normality.} -->
<!-- {#fig-simYtdiag3} -->
<!-- \end{figure} -->
<!-- \FloatBarrier -->

<!-- Forecast $h = 10$ steps ahead, with 95\% prediction intervals (@fig-simYtfcst}). -->
...
<!-- ```{r} -->
<!-- h = 10 -->
<!-- alpha = 0.05 #significance level alpha = 1 - confidence -->
<!-- PredObject <- predict(mod2, n.ahead = h) -->
<!-- pr_low <- PredObject$pred - qnorm(1 - alpha/2) * PredObject$se -->
<!-- pr_up  <- PredObject$pred + qnorm(1 - alpha/2) * PredObject$se -->
<!-- plot.ts(c(Yt, rep(NA, h)), las = 1, col = "grey50", lwd = 2, type = "o", ylab = "Yt") -->
<!-- lines(PredObject$pred, col = "red", lwd = 1) -->
<!-- lines(pr_low, col = "blue") -->
<!-- lines(pr_up, col = "blue") -->
<!-- @ -->
<!-- \caption{Observed time series $Y_t$ (gray) and predictions from ARMA(\Sexpr{p},\Sexpr{q}) model.} -->
<!-- {#fig-simYtfcst} -->
<!-- \end{figure} -->
...
<!-- We face the choice of different methods to estimate (i.e., estimators) and methods to compare models. Most often used estimators for ARMA models are maximum likelihood, Yule--Walker, and Burg; OLS is used less frequently. Methods to compare competing models may involve log-likelihood, MAE, RMSE and information criteria to assess in-sample (training) goodness-of-fit; diagnostics checks of residuals (discard models if their residuals do not pass the tests); comparisons of models on a testing set (PMAE, PRMSE, coverage and width of prediction intervals). For each of these options, there are multiple R functions available or can be written from scratch. -->

<!-- We have just considered a thorough approach to identifying the orders $p$ and $q$ using the ACF and PACF plots. The initially suggested model is compared with alternative plausible specifications (other $p$ and $q$ that may correspond to the same sample ACF and PACF). The resulting model can be selected using some quantitative criterion, such as AIC, BIC, or log-likelihood. This is a correct approach, but if many time series shall be processed at the same time, we can opt for using automatic model selection. -->

<!-- For example, \texttt{stats::ar} function by default uses Yule--Walker estimator and AIC to select the best order for AR($p$) model, where maximum of $p$ can be set by user: -->

<!-- ```{r} -->
<!-- ar(Yt, order.max = 11) -->
<!-- @ -->

<!-- The function \texttt{funtimes::ARest} \citep{R-funtimes} is a wrapper for \texttt{ar}. It makes the choice using BIC and has an additional method for estimating the coefficients, difference-based estimator by \citet{Hall:VanKeilegom:2003}, which is used by default: -->

<!-- ```{r} -->
<!-- funtimes::ARest(Yt) -->
<!-- # To compare with ar estimates, change the method and fix the order -->
<!-- # (turn off automatic selection): -->
<!-- funtimes::ARest(Yt, ar.method = "yw", ar.order = 2, BIC = FALSE) -->
<!-- @ -->

<!-- \paragraph{Note} Remember that BIC selects more parsimonious models than AIC does. Such simpler models are usually preferred for forecasting tasks. -->

<!-- Another useful function is \texttt{forecast::auto.arima} \citep{R-forecast}, which searches over possible models within the order constraints provided (the default maximal orders and search strategy are different from the two functions above -- see help files for each function in R): -->
...
<!-- The function \texttt{forecast::auto.arima} is probably the most comprehensive among the mentioned R functions for estimating and selecting ARMA models. -->

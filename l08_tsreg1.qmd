...
\begin{enumerate}
\item Learn three alternative ways of handling trends and/or seasonality to avoid spurious results: incorporate time effects into regression model, use deviations from trend or differenced series.
\item Introduce the concept of cointegration, learn how to detect it and model using an error correction model.
\end{enumerate}


\paragraph{Reading materials}
\begin{itemize}
\item Earlier materials on detrending individual time series
\item Chapter 6 in \citet{Kirchgassner:Wolters:2007} on cointegration
\end{itemize}
...
\section{Common approaches to regressing time series with trends}
...
\subsection{Incorporate time effects}
...
\subsection{Use deviations from trends}
...
\subsection{Use differenced series}
...
\subsection{Wrong approach (do not repeat at home): spurious regression}
...
\subsection{Example: predicting sales of home appliances}
...
\begin{equation}\label{eq:dish_timetrend}
...
\begin{equation}\label{eq:dish_diff}
...
\subsection{Two-step Engle--Granger method}

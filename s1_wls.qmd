...
Based on @fig-wlsOLS, the OLS assumption of homoscedasticity is violated, because the observations deviate farther from the regression line at its ends (i.e., the variability of regression residuals is higher at the low and high values of the predictor).

```{r}
#| label: fig-wlsOLS
#| fig-cap: "Simulated data example with heteroscedasticity. The gray line represents the underlying model; the dashed line is obtained from the OLS fit."
...
```{r}
#| label: fig-wls
#| fig-cap: "Simulated data example with heteroscedasticity. The gray line represents the underlying model; the dashed line is obtained from the WLS fit."
...
$$
This includes OLS as the special case when all the weights $w_i = 1$ ($i=1,\ldots,n$). In the example above, $w_i=1/x^2_i$.
...
To apply @eq-wls in R, specify the argument `weights`, remember to take an inverse. Note that the coefficients are now labeled as expected.
...
First, use OLS to estimate the simple linear regression exploring dishwasher shipments (DISH) and private residential investments (RES) for a number of years.
...
The plot in @fig-dishOLS indicates that the variance might be decreasing with the higher investments.
...
See @sec-gls on the method of generalized least squares (GLS) that allows to account for autocorrelation in regression modeling.
